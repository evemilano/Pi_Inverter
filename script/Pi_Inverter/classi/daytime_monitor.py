...
class DaytimeMonitor:
    ...
    def update(self):
        ...
        # --- Letture Modbus con connessione condivisa ---
        solar_power = 0
        grid_power = 0.0
        client = self.inverter_monitor.create_client()
        try:
            if client.connect():
                # Leggi potenza solare (registro 32080)
                solar_power = self.inverter_monitor.query_inverter(client=client) or 0
                time.sleep(0.5)  # Cooldown per il SDongle
                # Leggi potenza dalla rete (registro 37113)
                grid_power = self.apc_monitor.get_power_consumption(client=client)
            else:
                print("Connessione all'inverter fallita nel ciclo diurno")
        finally:
            client.close()

        # --- Da qui in poi: CSV, display, animazioni ---
